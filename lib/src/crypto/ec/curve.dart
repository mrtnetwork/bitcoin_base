...
class CustomECDSASigner implements Signer {
  ...
  CustomECDSASigner([this._digest, this._kMac, this._entry]);
...
class _RFC6979KCalculator {
  ...
  _RFC6979KCalculator(this._mac, this._n, BigInt d, Uint8List message,
      {Uint8List? entryPointes}) {
    _V = Uint8List(_mac.macSize);
    _K = Uint8List(_mac.macSize);
    _init(d, message, entryPointes);
  }

  void _init(BigInt d, Uint8List message, Uint8List? entryPointes) {
    ...
    if (entryPointes != null) {
      _mac.update(entryPointes, 0, entryPointes.length);
    }
    _mac.doFinal(_K, 0);
